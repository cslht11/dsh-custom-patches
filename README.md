...
**推荐**（自动诊断版本、检测官方是否已内置功能、备份、应用，`-y` 跳过确认）：
...
脚本会自动：
1. 定位 DSH 安装目录，读取本地版本并查询 npm 官方最新版
2. 校验版本是否为 `0.1.0-rc.7`（不匹配会拒绝并提示先升级）
3. **检测官方是否已内置我们的功能**——若目标文件已含功能标记（如官方新版把功能收编了），自动跳过对应补丁，避免重复/冲突
4. 对需要应用的补丁逐一备份（生成 `.bak`）并应用
5. 汇总报告 + 提示重启

> `install-dsh-custom.sh` 是 `apply-dsh-patches.sh` 的升级版（多出版本诊断与内置检测）。两者等效地应用同一套补丁；只需用其中一个即可，推荐前者。
